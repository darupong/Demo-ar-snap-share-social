...
const tips = [
  { icon: Sun, text: 'ใช้แสงสว่างเพียงพอ — สิ่งของชัดเจนช่วยให้ตรวจจับแม่นขึ้น' },
  { icon: Box, text: 'ลองส่องของบนโต๊ะ เช่น แก้วน้ำ หนังสือ กล่อง หรือคีย์บอร์ด' },
  { icon: Timer, text: 'เล่นเกมหาไอเท็มด้วยตัวเอง — ตั้งเวลาในใจแล้วหาของให้ทัน' },
  { icon: Camera, text: 'กดปุ่มวงกลมสีขาวตรงกลางเพื่อถ่ายภาพที่ตรวจเจอวัตถุ' },
  { icon: Share2, text: 'แชร์รูปไปยัง Facebook, Line หรือ Instagram' },
]

export interface ObjectDetectInstructionsProps {
  open: boolean
  onClose: () => void
}

/** Side sheet with object detection usage tips */
export default function ObjectDetectInstructions({ open, onClose }: ObjectDetectInstructionsProps) {
  if (!open) return null
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-cyan-500/20">
          <h2 className="text-white font-semibold text-lg">วิธีใช้ Object Detector</h2>
          <Button
            variant="ghost"
            size="icon"
            onClick={onClose}
        ...
        <div className="p-5 space-y-6 flex-1">
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="w-full bg-black rounded-2xl border border-cyan-500/20 flex items-center justify-center py-8">
              <div className="text-center space-y-2">
                ...
                <p className="text-cyan-400/60 text-xs">ส่องกล้องไปรอบ ๆ เพื่อให้ระบบหาวัตถุ</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-cyan-500/10 rounded-xl p-4 border border-cyan-500/20">
            ...
            <p className="text-gray-400 text-xs leading-relaxed">
              MediaPipe Image Classifier จะขึ้นชื่อวัตถุแบบ real-time
              เช่น bottle, book, chair พร้อมเปอร์เซ็นต์ความมั่นใจ
            </p>
          </div>
        </div>
      </div>
